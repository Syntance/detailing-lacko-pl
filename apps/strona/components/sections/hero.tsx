...
export function Hero({
  ...
  const headline =
    "Detailing samochodowy w Łącku — ceny z góry, płacisz po efekcie";
  const description =
    "Pranie tapicerki, kompleksowe czyszczenie wnętrza, polerowanie lakieru i reflektorów. Pełny cennik niżej, zdjęcia przed/po. Czerniec 72, terminy po 16:00 i w weekendy.";
    ...
    <section id="hero" aria-label="Detailing Łącko" className="relative">
      <div className="mx-auto grid w-full max-w-6xl grid-cols-1 items-center gap-14 px-6 pt-28 pb-16 lg:grid-cols-2 lg:gap-28 lg:pt-40 lg:pb-24">
        <div className="relative order-2 flex flex-col justify-center lg:order-1">
          <h1 className="hero-enter max-w-xl font-serif text-4xl leading-[1.08] font-medium text-balance md:text-5xl lg:text-[3.4rem]">
            {headline}
          </h1>
          <p className="hero-enter mt-5 max-w-lg text-lg text-pretty text-muted-foreground md:text-xl [animation-delay:90ms]">
            {description}
          </p>

          {/* Linia zaufania — pełne odwrócenie ryzyka (rdzeń UVP v3). */}
          <p className="hero-enter mt-5 max-w-lg font-medium text-pretty [animation-delay:180ms]">
            Wiesz, ile zapłacisz, zanim zadzwonisz. I płacisz dopiero, gdy
            zobaczysz efekt.
          </p>
          ...
          {/* Pasek zaufania — trzy dowody z UVP, bez ocen „na słowo". */}
          <p className="hero-enter mt-6 max-w-lg text-sm text-pretty text-muted-foreground [animation-delay:360ms]">
            pełny cennik bez „od" · zdjęcia przed/po każdej realizacji ·
            płatność gotówka/BLIK po obejrzeniu efektu
          </p>
        </div>
