...
4. ENV Medusa (obrazy):
   - `S3_*` / R2 — uploady z panelu trafiają na CDN (`cms-uploads/`)
   - **Storefront (Vercel):** `S3_FILE_URL` + `NEXT_PUBLIC_S3_FILE_URL` (ta sama publiczna baza R2)

5. Route Handlers uploadu (wzorzec lumine — montuj w `app/api/`):

```ts
// app/api/magazyn/cms-upload/route.ts
export { POST, runtime, maxDuration } from "@moduly/magazyn-core/api/cms-upload";
```

Opcjonalnie w `moduly.config.ts`: `storage.cmsUploadApiPath` gdy ścieżka API ma być inna niż domyślna.
